...
This problem occurs when the Jupyter kernel unexpectedly
stops working while running code in a Jupyter notebook.
...
To resolve the "Kernel has died" issue, you need to create a new Conda environment.
Following the steps below will guide you through the process of creating an environment named "DTU" with the necessary packages installed.
...
Open VS Code and select `DTU(Python {{ python_version_recommended }}.XX)` as your Kernel.


.. note::
   If you need any help with how to select the kernel in VS Code or other related issues, please refer to the guide :ref:`here <learn-more.vscode.notebooks>`.
